...
const NavBar = () => {
    ...
    return (
        <div>
                ...
                <div className={'flex items-center gap-4'}>
                    ...
                    {user ? (
                        <div className={'flex flex-col'}>
                            <div onClick={() => setOpenMenu(!openMenu)} className={'h-10 w-10 rounded-full border-2 border-secondary-dark overflow-hidden'}>
                                <img src={'/images/ProfilePic.png'} className={'w-full h-full object-cover object-center'} alt="ProfilePic"/>
            ...
            {openMenu &&(
                <div className={'flex flex-col border border-secondary-dark bg-background-dark text-white rounded-lg right-0 w-72 absolute z-50 px-6 py-2'}>
                    <Link href={'/userProfile'} className={'border-b border-gray-300 p-3'}>Profile</Link>
                    <Link href={'/'} className={'border-b border-gray-300 p-3'}>Account Settings</Link>
                    <p onClick={logout} className={'border-b border-gray-300 p-3 text-red-500'}>Sign Out</p>
                </div>
            )}
        </div>
    )
